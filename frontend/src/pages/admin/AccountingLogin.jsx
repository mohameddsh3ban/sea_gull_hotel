...
const AccountingLogin = ({ onLogin }) => {
  ...
  const from = location.state?.from || '/accounting/dashboard';

  const handleLogin = async (e) => {
    e?.preventDefault();
    if (!password.trim()) {
      setError('Please enter your accounting password.');
      return;
    }
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE}/accounting/login`, {
        method: 'POST',
      ...
      if (res.ok && data.success) {
        sessionStorage.setItem('accountingToken', password);
        onLogin?.();
  ...
  return (
    // No page wrapper here—AuthLayout provides the card + centering
    <form onSubmit={handleLogin} className="space-y-6">
      {/* Optional small title; remove if you want totally minimal */}
      <h2 className="text-lg font-semibold text-gray-800">Accounting Portal</h2>
