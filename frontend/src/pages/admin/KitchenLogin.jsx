...
const KitchenLogin = ({ onLogin }) => {
  ...
  const from = location.state?.from || '/kitchen/dashboard';

  const handleLogin = async (e) => {
    e?.preventDefault();
    if (!password.trim()) {
      setError('Please enter your kitchen password.');
      return;
    }
    ...
    setError('');
    try {
      const res = await fetch(`${API_BASE}/kitchen/login`, {
        method: 'POST',
      ...
      if (res.ok && data.success) {
        sessionStorage.setItem('kitchenToken', password);
        onLogin?.();
  ...
  return (
    // No full-page wrapper — AuthLayout handles layout
    <form onSubmit={handleLogin} className="space-y-6">
      <h2 className="text-xl font-semibold text-gray-800">Kitchen Portal</h2>
